...
import {markerAdd, markersLoad} from '../ActionCreators/markerActions';
...
import {commentsLoad} from '../ActionCreators/commentsActions';
...
export default class App extends PureComponent {
  ...
  loadM() {
    ...
    loadMarkers().then((markers) => {
      console.log('MARKERS', MARKERS_URL, markers);
      store.dispatch(markersLoad(markers));
      this.setState({
        markers: markers});
    }).catch((error) => {
  ...
  loadComms() {
    ...
    loadComments().then((comments) => {
      ...
      store.dispatch(commentsLoad(comments));
    }).catch((error) => {
      console.log(error.response);
      this.addNotification('danger', 'ERROR', error.response);
    });
  ...
  AddMarker = (e) => {
    ...
    markers.push(newEl);
    store.dispatch(markerAdd(
        {
          lat: this.state.tmpLat.toString(),
          lng: this.state.tmpLng.toString(),
          text: this.state.tmpName}));
    this.saveM(newEl);
    this.setState({
      markers: markers,
